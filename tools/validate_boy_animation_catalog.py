#!/usr/bin/env python3
"""Catalog all 52 Boy animations and validate exactly index 43 / ID 1069."""
...
from jfg_re.boy_animation_catalog import export_catalog_and_selected
...
def main() -> int:
    ...
    try:
        report = export_catalog_and_selected(args.boy, args.rom, args.texture_manifest, output)
    except (OSError, FileExistsError, BoyExportError, RomIdentityError) as error:
        print(f"Boy animation catalog validation aborted: {error}", file=sys.stderr)
        return 2
    print(json.dumps({"output": str(output), "selection": report["selection"], "times": report["selected_times"]}, indent=2))
    return 0
