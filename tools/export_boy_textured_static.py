#!/usr/bin/env python3
"""Export Prop 220 Boy as a pinned textured static Blender validation OBJ."""
...
from jfg_re.boy_textured_export import export_textured_boy
...
def main() -> int:
    ...
    try:
        report = export_textured_boy(args.boy, args.rom, args.texture_manifest, output)
    except (OSError, FileExistsError, BoyExportError, RomIdentityError) as error:
        print(f"Textured Boy export aborted: {error}", file=sys.stderr)
        return 2
    print(json.dumps({"output": str(output), **report["counts"], "uv_range": report["uv_range"]}, indent=2))
    return 0
