#!/usr/bin/env python3
"""Export pinned temporal snapshots for Prop 220 Boy Animation 0 only."""
...
from jfg_re.boy_anim0_temporal import export_boy_anim0_temporal
...
def main() -> int:
    ...
    try:
        report = export_boy_anim0_temporal(args.boy, args.rom, args.texture_manifest, output)
    except (OSError, FileExistsError, BoyExportError, RomIdentityError) as error:
        print(f"Boy Animation-0 temporal export aborted: {error}", file=sys.stderr)
        return 2
    print(json.dumps({"output": str(output), "selected_times": report["selected_times"]}, indent=2))
    return 0
