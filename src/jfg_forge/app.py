...
def _arguments(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Open the JFG Forge Boy/Juno model viewer.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        message = f"JFG Forge could not load Boy / Prop 220:\n\n{error}"
        print(message, file=sys.stderr)
        QMessageBox.critical(None, "JFG Forge load error", message)
        return 1
    window = MainWindow(boy, scene, render_data, information)
    window.show()
